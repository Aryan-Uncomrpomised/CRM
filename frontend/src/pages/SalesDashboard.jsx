...
export default function SalesDashboard() {
  ...
  const { data: pnlPartners = [] } = useQuery({
    ...
    queryFn: async () =>
      (
        await api.get("/stats/pnl-partners", {
          params: {
            category: categoryFilter,
            q: pnlSearch || undefined,
            sortBy: sortBy,
          },
        })
      ).data,
  ...
  // Filtered total calculation directly from server-side pnlPartners
  const filteredTotalSpent = useMemo(
    () => pnlPartners.reduce((acc, c) => acc + (c.total_spent || 0), 0),
    [pnlPartners]
  );

  // Filter & sort Top Products
  const filteredProducts = useMemo(() => {
    let prods = s.top_products || [];
    if (prodSearch.trim()) {
      const q = prodSearch.toLowerCase();
      prods = prods.filter((p) => p.name?.toLowerCase().includes(q));
    }
    return [...prods].sort((a, b) => {
      if (prodSort === "revenue_desc") return (b.revenue || 0) - (a.revenue || 0);
      if (prodSort === "qty_desc") return (b.quantity || 0) - (a.quantity || 0);
      if (prodSort === "name_asc") return (a.name || "").localeCompare(b.name || "");
      return 0;
    });
  }, [s.top_products, prodSearch, prodSort]);
  ...
  return (
    <div className="p-8 space-y-8" data-testid="sales-dashboard-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Top Products Leaderboard with Search & Sort */}
        <div className="border border-white/[0.08] rounded-xl p-6 bg-[color:var(--vc-surface)] flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[11px] font-mono uppercase tracking-widest text-white/40">
            ...
            <div className="mt-4 flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className="pl-8 h-8 bg-white/[0.03] border-white/10 text-xs"
                />
              </div>
              <select
                value={prodSort}
                onChange={(e) => setProdSort(e.target.value)}
                className="h-8 bg-white/[0.03] border border-white/10 rounded px-2 text-[11px] font-mono text-white/80 focus:outline-none"
              >
                <option value="revenue_desc" className="bg-neutral-900">Revenue (High to Low)</option>
                <option value="qty_desc" className="bg-neutral-900">Qty (High to Low)</option>
            ...
            <div className="mt-4 space-y-2.5 max-h-72 overflow-y-auto pr-1">
              {filteredProducts.slice(0, 8).map((p, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between p-2.5 rounded-lg bg-white/[0.02] border border-white/[0.04] hover:border-white/10 transition-colors"
                >
          ...
          <div className="pt-3 border-t border-white/[0.06] text-center text-xs text-white/40 font-mono">
      ...
      <div className="border border-white/[0.08] rounded-xl p-6 bg-[color:var(--vc-surface)] space-y-6">
        {/* Section Header */}
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <div className="font-display font-bold text-2xl mt-0.5">
              Customer Partner Spent & Order Ledger ({pnlPartners.length} Partners)
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs font-mono text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded border border-emerald-500/20">
              Filtered Total: {format(filteredTotalSpent, { digits: 0 })}
            </span>
          </div>
        </div>

        {/* TOOLBAR: Section Tabs, Search & Sort Controls */}
        <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl bg-white/[0.02] border border-white/[0.06]">
          {/* Section Filter Pills */}
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[11px] font-mono uppercase text-white/40 mr-1 flex items-center gap-1">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="pl-8 w-56 h-9 bg-white/[0.03] border-white/10 text-xs"
              />
            </div>
            ...
            <div className="flex items-center gap-1.5 bg-white/[0.03] border border-white/10 rounded-lg px-2.5 py-1">
              ...
              <select
                ...
                className="bg-transparent text-xs font-mono text-white/90 focus:outline-none cursor-pointer py-1"
              >
                <option value="spent_desc" className="bg-neutral-900">Spent: High to Low</option>
                <option value="spent_asc" className="bg-neutral-900">Spent: Low to High</option>
                <option value="orders_desc" className="bg-neutral-900">Orders: Most to Least</option>
                <option value="orders_asc" className="bg-neutral-900">Orders: Least to Most</option>
                <option value="name_asc" className="bg-neutral-900">Partner Name (A-Z)</option>
                <option value="date_desc" className="bg-neutral-900">Latest Purchase Date</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody className="divide-y divide-white/[0.04]">
              {pnlPartners.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-white/40 font-mono text-xs">
                    No partner records match the selected section & sort criteria.
                  </td>
                </tr>
              ) : (
                pnlPartners.map((c, i) => {
                        ...
                        <div>
                          <div className="text-white text-sm font-semibold flex items-center gap-2">
                            ...
                            {c.tags && c.tags.length > 0 && (
                              <span className="text-[9.5px] px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-300 font-mono font-medium">
                                {c.tags[0]}
                              </span>
                            )}
                          </div>
