...
export default function AppShell() {
  ...
  return (
    <div className="min-h-screen flex bg-[color:var(--vc-bg)] text-white">
      ...
      {/* ── Sidebar ─────────────────────────────────────────────── */}
      <aside className="w-60 shrink-0 border-r border-white/[0.06] bg-[color:var(--vc-bg)] flex flex-col h-screen sticky top-0">

        {/* Logo */}
        <div className="px-5 py-5 flex items-center gap-2 border-b border-white/[0.06] shrink-0">
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-4 space-y-0.5 min-h-0">
          {NAV.map((n) => (
            <NavLink
              key={n.to}
              to={n.to}
        ...
        <div className="shrink-0 border-t border-white/[0.06] px-3 py-3 space-y-1">
          ...
          >
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[color:var(--vc-accent)] to-[color:var(--vc-lime)] flex items-center justify-center font-display font-black text-black text-sm shrink-0">
          ...
          {/* Sign out — always visible, never hidden in dropdown */}
          <button
            data-testid="menu-logout"
            onClick={onLogout}
            className="w-full flex items-center gap-2.5 px-3 py-2 rounded-md text-[13px] font-medium text-red-400 hover:bg-red-950/30 hover:text-red-300 transition-colors"
          >
